...
*/
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        surfaceParams.addRule(RelativeLayout.CENTER_IN_PARENT, RelativeLayout.TRUE);
        ...
        // (openmw) don't let SDL toggle immersive mode; the port manages
        // system UI visibility itself
    ...
    // (openmw) virtual cursor / on-screen controls hooks, implemented in
    // libopenmw.so (apps/openmw/androidmain.cpp)
...
*/
    ...
    // (openmw) used by the virtual cursor overlay to translate touches
    ...
    static class ShowTextInputTask implements Runnable {
        ...
        @Override
        public void run() {
            // (openmw) the port ships its own on-screen keyboard; never show
            // SDL's DummyEdit soft input
            /*
...
//        }
    ...
    protected void messageboxCreateAndShow(Bundle args) {
        // (openmw) single-button boxes are fatal-error dialogs; the custom
        // themed dialog renders them unreliably, use a plain AlertDialog
        if (args.getIntArray("buttonIds").length <= 1) {
            showModernDialog(args).show();
            return;
        }
    ...
    // (openmw) plain AlertDialog for single-button (fatal error) messageboxes
...
class SDLMain implements Runnable {
    ...
    public void run() {
        // Runs SDL_main()
        // (openmw) initialize JNI bindings before the engine thread starts
