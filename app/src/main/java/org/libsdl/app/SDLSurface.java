...
public class SDLSurface extends SurfaceView implements SurfaceHolder.Callback,
    ...
    // (openmw) fixed render resolution from the launcher settings; 0 = native
    ...
    // (openmw) letterbox the fixed render resolution into the view
    @Override
    ...
    public void surfaceChanged(SurfaceHolder holder,
        ...
        // (openmw) with a fixed render resolution, mWidth/mHeight come from
        // onMeasure (the letterboxed view size), not the surface buffer size
        if (fixedWidth <= 0) {
            mWidth = width;
            mHeight = height;
        }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        // (openmw) drop event unless the mouse is shown (i.e. unless we're in
        // a mouse-enabled menu)
        if ((action == MotionEvent.ACTION_DOWN || action == MotionEvent.ACTION_POINTER_DOWN) &&
                SDLActivity.isMouseShown() == 0)
            return false;

        // (openmw) in joystick mouse mode don't process touch events at all
